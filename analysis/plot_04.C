...
void plot_04(){
	...
	VariableNameCollection[1]	="SecondariesSpectrumAtVertex";
	VariableNameCollection[2]	="SecondariesSpectrumAtNPSurface";
	...
	for (FileCollectionIt = FileCollection.begin() ; FileCollectionIt != FileCollection.end(); ++FileCollectionIt){
		...
		h[FileCollectionIt->first]->SetAxisRange(1E-7,MaximunBinContent*1.05,"Y");
		...
		h[FileCollectionIt->first]->SetXTitle("Secondary e- energy at vertex [keV]");
		h[FileCollectionIt->first]->SetYTitle("Relative counts [%]");
		h[FileCollectionIt->first]->SetLineWidth(2.0);
	}

	// --- End SecondariesSpectrumAtVertex ---

	// --- SecondariesSpectrumAtNPSurface ---
	...
	for (FileCollectionIt = FileCollection.begin() ; FileCollectionIt != FileCollection.end(); ++FileCollectionIt){
		...
		h[FileCollectionIt->first]->SetTitle("");
		h[FileCollectionIt->first]->SetXTitle("Secondary e- energy at NP surface [keV]");
		h[FileCollectionIt->first]->SetYTitle("Relative counts [%]");
		h[FileCollectionIt->first]->SetLineWidth(2.0);
	}

	// --- End SecondariesSpectrumAtNPSurface ---

	c1->Print("data/plot_04.png");
