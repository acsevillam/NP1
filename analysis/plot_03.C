...
void plot_03(){
	...
	VariableNameCollection[1]	="NumberOfElectronsPerGoodEvent";
	...
	for (FileCollectionIt = FileCollection.begin() ; FileCollectionIt != FileCollection.end(); ++FileCollectionIt){
		...
		h[FileCollectionIt->first]->SetXTitle("No. of secondary e- per good event");
		h[FileCollectionIt->first]->SetYTitle("Relative counts [%]");
		h[FileCollectionIt->first]->SetLineWidth(2.0);
	}

	// --- End NumberOfElectronsPerGoodEvent ---

	c1->Print("data/plot_03.png");
